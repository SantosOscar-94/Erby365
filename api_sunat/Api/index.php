...
require '../Xml/facturacion_3_1.php';
...
function FacturacionUBL($Json_In)
{
    ...
    */
    ...
    }elseif ($TipoComprobante == "36") // Comprobante de detracción
    {
        $instancia  = new cpeFacturacionUBL3_1();
        $array      = $instancia->Registrar_FacturacionUBL3_1($Json_Fac, $Json_Emp); // General el XML
        if ($array[0] == 'Registrado') {
            $firmado = new firmadocpe();
            $arrayFirma = $firmado->FirmarCpe($Json_Fac, $Json_Emp); // Firmar el XML
            if ($arrayFirma[0] == 'Registrado') 
            {
                if ($EnvioSunat == true) 
                {
                    ...
                    if ($arrayEnviar[0] == 'Error1' ||$arrayEnviar[0] == 'Error2' ||$arrayEnviar[0] == 'Error3') 
                    {
                        $error_out[0]      = Error_Out();
                        echo json_encode($arrayEnviar[0]);
                    } 
                    else 
                    {
                        $respuesta_servidor[0] = RespuestaServidor($Json_Emp, $Json_Fac, "Enviado", "Registrado");
    ...
    {
        echo json_encode(['status' => false, 'msg' => 'No se registró']);
    }
}
