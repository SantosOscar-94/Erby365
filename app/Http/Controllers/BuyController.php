...
class BuyController extends Controller
{
        ...
        if (!empty($cart['products'])) {
            foreach ($cart['products'] as $i => $product) {
                ...
                $html_cart .= '<tr>
                                ...
                                <td class="text-center"><input type="text" class="form-control form-control-sm text-center input-update" value="' . number_format($product["precio_compra"], 2, ".", "") . '" data-precio="' . number_format($product["precio_compra"], 2, ".", "") . '" data-cantidad="' . $product["cantidad"] . '" data-id="' . $product["id"] . '" name="precio_compra"></td>
                                <td class="text-center"> <span class="total" data-total="'.($product["precio_compra"] * $product["cantidad"]).'"' . $product["id"] . '"> ' . number_format(($product["precio_compra"] * $product["cantidad"]), 2, ".", "") . '</span></td>
    ...
    public function save(Request $request)
    {
        ...
        foreach ($cart['products'] as $product) {
            ...
            Kardex::create([
                'documentTypeId'    => $idtipo_comprobante,
                'userId'            => auth()->user()->id,
