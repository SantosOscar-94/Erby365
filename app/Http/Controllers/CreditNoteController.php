...
class CreditNoteController extends Controller
{
    ...
    public function save(Request $request)
    {
        ...
        if(ArchingCash::where([['idcaja', Auth::user()['idcaja']], ['idusuario', Auth::user()['id']]])->latest('id')->first()['estado'] == 2){
            echo json_encode([
                'status'    => false,
                'msg'       => 'No es posible realizar esta acción en una caja cerrada',
                'type'      => 'warning'
            ]);
            return;
        }
        ...
        $cant = json_decode($request->input('cant'), true);
        ...
        foreach($detalle as $product)
        {
            foreach($cant as $cantidad){
                if($product['id'] == $cantidad['id']){
                    foreach($price as $precio){
                        //dd($product['id'], $cantidad['id'], $precio['id']);
                        if($product['id'] == $precio['id']){
                            DetailBilling::create([
                                ...
                                'precio_unitario'       => $precio['price'],
                                'precio_total'          => ($precio['price'] * $cantidad['cant'])
                            ]);
                        }
                    }
                }
