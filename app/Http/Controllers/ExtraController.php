    ...
    public function send_voucher(Request $request)
    {
        ...
        switch ($type_document) {
            case 'billing':
                ...
                $url_server                 = 'http://comprobantes.demoprueba.erby365.com/';
                $enlace                     = $url_server . $data["name"] . '.pdf';
                $cid                        = ftp_connect("50.116.27.100");
                $resultado                  = ftp_login($cid, 'empredeg', 'f32hYviY98');
                if ((!$cid) || (!$resultado)) {
                    echo json_encode([
                        'status'    => false,
                ...
                ftp_pasv($cid, true);
                ftp_chdir($cid, "comprobantes.demoprueba.erby365.com/");
                ftp_put(
                    $cid,
                    '/comprobantes.demoprueba.erby365.com/' . $data["name"] . '.pdf',
