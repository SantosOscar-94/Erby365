...
use App\Models\DetailGRTransportista;
use App\Models\GRTransportista;
use App\Models\Product;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;

class GRTransportistaController extends Controller
{
    public function index()
    {
        return view('admin.document_avanzados.guias.gr_transportista.home');
    }

    public function add()
    {
        return view('admin.document_avanzados.guias.gr_transportista.add');
    }

    public function get()
    {
        $rows = GRTransportista::all();

        return Datatables()
            ->of($rows)
            ->addColumn('sender', function ($rows) {
                return Client::find($rows->sender_id)->nombres ?? "Sin especificar";
            })
            ->addColumn('recipient', function ($rows) {
                return Client::find($rows->destination_pointId)->nombres ?? "Sin especificar";
            })
    ...
    public function save(Request $request)
    {
        ...
        if (empty(session()->get('gr_transportista')['products'])){
            echo json_encode([
                'status' => false,
...
//                'punto-partida'     => 'required|integer',
//                'destino'           => 'required|integer',
//                'punto-llegada'     => 'required|integer',
//                'datos-vehiculo'    => 'required|integer',
//                'datos-conductor'   => 'required|integer',
//                'datos-vehiculo-secundario'     => 'nullable|integer',
//                'datos-conductor-secundario'    => 'nullable|integer',
                'establecimiento'   => 'integer',
                'fecha-emision'     => 'date',
                'fecha-traslado'    => 'date',
                'unidad-medida'     => 'integer',
                'peso-total'        => 'numeric',
                'observaciones'     => 'nullable|string',
                'orden-compra'      => 'nullable|string|max:255',
                'empresa'           => 'integer',
                'remitente'         => 'integer',
                'punto-partida'     => 'integer',
                'destino'           => 'integer',
                'punto-llegada'     => 'integer',
                'datos-vehiculo'    => 'integer',
                'datos-conductor'   => 'integer',
                'datos-vehiculo-secundario'     => 'nullable|integer',
                'datos-conductor-secundario'    => 'nullable|integer',
            ], [
        ...
        GRTransportista::create([
            ...
            'purchase_order'    => $validatedData['orden-compra'],
            'bussinesId'        => $validatedData['empresa'],
            'customerId'        => $validatedData['remitente'],
            'senderId'          => $validatedData['remitente'], // Asumiendo que remitente es el senderId
            'start_pointId'     => $validatedData['punto-partida'],
            'destination_pointId' => $validatedData['destino'],
            'end_pointId'       => $validatedData['punto-llegada'],
            'vehicleId'         => $validatedData['datos-vehiculo'],
            'driverId'          => $validatedData['datos-conductor'],
            'vehicle2Id'        => $validatedData['datos-vehiculo-secundario'], // Puede ser null
            'driver2Id'         => $validatedData['datos-conductor-secundario'], // Puede ser null
        ]);

        $gr_transportistaId = GRTransportista::latest()->first()->id;

        foreach (session()->get('gr_transportista')['products'] as $product) {
            DetailGRTransportista::create([
                'gr_transportistaId' => $gr_transportistaId,
    ...
    public function create_cart()
    {
        $sale_note =
            [
                'gr_transportista' =>
                    [
                        'products'     => session('gr_transportista')['products'] ?? null
                    ]
            ];

        session($sale_note);
        return session()->get('gr_transportista');
    }
    ...
    public function add_product_cart($id, $cantidad, $peso, $opcion)
    {
        ...
        if (empty(session()->get('gr_transportista')['products'])) {
            session()->push('gr_transportista.products', $new_product);
            return true;
        }

        foreach (session()->get('gr_transportista')['products'] as $index => $product) {
            if ($id == $product['id'] && $product['opcion'] == $opcion) {
                ...
                session()->put('gr_transportista.products.' . $index, $product);
                return true;
            }
        }
    ...
    {
        if (!session()->get('gr_transportista') || empty(session()->get('gr_transportista')['products'])) {
            return false;
        }

        foreach (session()->get('gr_transportista')['products'] as $index => $product) {
            if ($id == $product['id'] && $product['opcion'] == $opcion) {
                session()->forget('gr_transportista.products.' . $index, $product);
                return true;
            }
        }
    }
    ...
    public function destroy_cart()
    {
        ...
        session()->forget('gr_transportista');
        return true;
    }
}
