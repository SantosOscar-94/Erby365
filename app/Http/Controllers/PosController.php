...
class PosController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['modo_pagos']         = PayMode::where([['id', '!=', 1], ['id', '!=', 8]])->get();
        ...
        $data['cuentas']            = Cuentas::all();
        $data['type_documents_p']   = TypeDocument::where('estado', 1)->limit(2)->get();
        $data['products']           = Product::get();

        $data["users"]          = User::all();
        $data["type_documents2"] = TypeDocument::where('id', 1)->orWhere([['id', 2], ['id', 7], ['id', 6]])->get();
    ...
    public function load_cart(Request $request)
    {
        ...
        if ($request['cambio'] == 1) {
            ...
            $html_totales   .= '<div class="d-flex justify-content-between align-items-center mt-3">
                                    <p class="mb-0">OP. Gravadas</p>
                                    <h6 class="mb-0"><span name="moneda">$ </span><span class="gravadas" data-valor="'.number_format((($cart['exonerada'] + $cart['gravada'] + $cart['inafecta'])), 2, ".", "").'">' . number_format((($cart['exonerada'] + $cart['gravada'] + $cart['inafecta']) / $request['cambio']), 2, ".", "") . '<span></h6>
                                </div>
                                <div class="d-flex justify-content-between align-items-center mt-3">
                                    <p class="mb-0">IGV</p>
                                    <h6 class="mb-0"><span name="moneda">$ </span><span class="igv" data-valor="'.number_format($cart['igv'], 2, ".", "").'">' . number_format(($cart['igv'] / $request['cambio']), 2, ".", "") . '<span></h6>
                                </div>
                                <hr>
                                <div class="d-flex justify-content-between align-items-center mt-3 pb-1">
                                    <p class="mb-0">Total</p>
                                    <h6 class="mb-0"><span name="moneda">$ </span><span class="total2" id="total2" data-valor="'.number_format($cart['total'], 2, ".", "").'">' . number_format(($cart['total'] / $request['cambio']), 2, ".", "") . '<span></h6>
                                </div>';
        }
...
//        $correlativo            = $serie_sale[1];
        ...
        if ($iddocumento_tipo == "7") // NV
        {
            ...
            foreach ($cart['products'] as $product) {
                ...
                if($product["opcion"] == 1)
                {
                    ...
                    Kardex::create([
                        'documentTypeId'    => $iddocumento_tipo, //$product['idtipo_comprobante'],
                        'userId'            => auth()->user()->id,
    ...
    {
        $customPaper                = array(0, 0, 630.00, 210.00);
        $data['cambio']             = session()->get('cambio');
        $data['business']           = Business::where('id', 1)->first();
        $data['ubigeo']             = $this->get_ubigeo($data['business']->ubigeo);
        $ruc                        = $data['business']->ruc;
        ...
        $formatter                  = new NumeroALetras();
        $data['numero_letras']      = $formatter->toWords($factura->total, 2);
        $data['numero_letras2']     = $formatter->toWords($this->redondeado($factura->total / $data['cambio']), 2);
        $data['tipo_comprobante']   = TypeDocument::where('id', $factura->idtipo_comprobante)->first();
        $data['vendedor']           = mb_strtoupper(User::where('id', $data['factura']->idusuario)->first()->user);
        $data['payment_modes']      = DetailPayment::select('detail_payments.*', 'pay_modes.descripcion as modo_pago')
            ->join('pay_modes', 'detail_payments.idpago', 'pay_modes.id')
