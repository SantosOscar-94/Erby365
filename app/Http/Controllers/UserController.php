...
class UserController extends Controller
{
    ...
    public function get()
    {
        $users     = User::select('users.*', 'cashes.descripcion as caja', 'warehouses.descripcion as almacen')
                        ->join('cashes', 'users.idcaja', 'cashes.id')     
                        ->join('warehouses', 'users.idalmacen', 'warehouses.id')           
                        ->orderBy('id', 'DESC')->get();
    ...
    public function save(Request $request)
    {
        ...
        $idalmacen          = trim($request->input('idalmacen'));
        User::create([
