<script>
    ...
    $('body').on('click', '.btn-confirm', function() {
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: "{{ route('admin.delete_conductores') }}",
                    method: 'POST',
                    data: {
                        '_token': "{{ csrf_token() }}",
                        id: id
    ...
    $('body').on('click', '.btn-detail', function() {
        event.preventDefault();
        $('#opcional_direccion_2').removeClass('d-none');
        let id = $(this).data('id');
        $.ajax({
            url: "{{ route('admin.detail_conductores') }}",
            method: 'POST',
            data: {
                '_token': "{{ csrf_token() }}",
                id: id
            ...
            success: function(r) {
                ...
                html_province = '';
                html_district = '';
                $(`#form_edit_client select[name="tipo_documento"] option[value="${r.conductores.iddoc}"]`)
                    .prop('selected', true);
                switch (r.conductores.iddoc) {
                    case 2:
                    ...
                    case 4:
                        if (r.client.dni_ruc != "10753330459")
                            $('#opcional_direccion_2').addClass('d-none');
                        $('#form_edit_client #wrapper-input-reniec').removeClass('d-none');
                ...
                $(`#form_edit_client input[name="id"]`).val(r.conductores.id);
                $(`#form_edit_client input[name="dni_ruc"]`).val(r.conductores.dni_ruc);
                $(`#form_edit_client input[name="razon_social"]`).val(r.conductores.nombres);
                $(`#form_edit_client input[name="direccion"]`).val(r.conductores.direccion);
                $(`#form_edit_client input[name="licencia"]`).val(r.conductores.licencia);
                $(`#form_edit_client input[name="telefono"]`).val(r.conductores.telefono);
    ...
    $('body').on('click', '#form_edit_client .btn-store-client', function() {
        ...
        let form = $('#form_edit_client').serialize(),
            ...
            licencia = $('#form_edit_client input[name="licencia"]');
        ...
        if (tipo_documento.val() != '0' && dni_ruc.val().trim() != '' &&
           licencia.val().trim() != '' &&
            ...
            $.ajax({
                url: "{{ route('admin.store_conductores') }}",
                method: 'POST',
                data: form,
                beforeSend: function() {
