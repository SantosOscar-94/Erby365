...
{{--        <div class="col-12">--}}
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">Agregar guía de remisión - Transportista</h4>
                </div>
                <div class="card-body">
                    <form class="row" id="form_gr_transportista">
                        @csrf
                        <div class="form-group col-2 mb-3">
                            <label for="establecimiento">Establecimiento *</label>
                            <select class="form-control select2" id="establecimiento" name="establecimiento" required>
                                <option value="0">Selecciona una oficina principal</option>
                        ...
                        <div class="form-group col-2 mb-3">
                            ...
                            <input type="date" class="form-control" id="fecha-traslado" name="fecha-traslado">
                        </div>
...
{{--                        <div class="form-group col-4 mb-3">--}}
{{--                            <label for="guias-remision">Guías de remisión relacionadas</label>--}}
{{--                            <a href="#" id="guias-remision">[Agregar]</a>--}}
{{--                        </div>--}}

                        <hr>
                        <h4>Pagador del flete</h4>

                        <div class="form-group col-6 mb-3">
                            <label for="empresa">Empresa [+ Nuevo]</label>
                        ...
                        <div class="form-group col-6 mb-3">
                            <label for="remitente">Remitente [+ Nuevo]</label>
                            <select class="form-control select2" id="remitente" name="remitente" required>
                        ...
                        <div class="form-group col-12 mb-3">
                            ...
                            <select class="form-control" id="punto-partida" name="punto-partida">
                                <option value="0">Seleccionar punto de partida</option>
                        ...
                        <div class="form-group col-6 mb-3">
                            <label for="destino">Destinatario [+ Nuevo]</label>
                        ...
                        <div class="form-group col-6 mb-3">
                            <label for="punto-llegada">Punto de llegada</label>
                            <select class="form-control" id="punto-llegada" name="punto-llegada">
                                <option value="0">Seleccionar punto de llegada</option>
                                @foreach(\App\Models\Direccion_Partida::all() as $item)
                                    <option value="{{$item->id}}">{{$item->ubigeo}} - {{$item->direccion}}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group col-6 mb-3">
                            <label for="datos-vehiculo">Datos del vehículo [+ Nuevo]</label>
                            <select class="form-control" id="datos-vehiculo" name="datos-vehiculo">
                        ...
                        <div class="form-group col-6 mb-3">
                            <label for="datos-conductor">Datos del conductor [+ Nuevo]</label>
                            <select class="form-control" id="datos-conductor" name="datos-conductor">
                        ...
                        <div class="form-group col-6 mb-3">
                            <label for="datos-vehiculo-secundario">Datos del vehículo secundario [+ Nuevo]</label>
                            <select class="form-control" id="datos-vehiculo-secundario" name="datos-vehiculo-secundario">
                                <option value="0">Seleccionar vehículo</option>
                                @foreach(\App\Models\Vehiculos::all() as $item)
                                    <option value="{{$item->id}}">{{$item->marca}} - {{$item->modelo}} - {{$item->num_placa}}</option>
                                @endforeach
                        ...
                        <div class="form-group col-6 mb-3">
                            <label for="datos-conductor-secundario">Datos del conductor secundario [+ Nuevo]</label>
                            <select class="form-control" id="datos-conductor-secundario" name="datos-conductor-secundario">
                                <option value="0">Seleccionar conductor</option>
                                @foreach(\App\Models\Conductores::all() as $item)
                                    <option value="{{$item->id}}">{{$item->dni_ruc}} - {{$item->nombres}}</option>
                                @endforeach
                    ...
                    <hr>
                            ...
                            <div class="col-md-12">
                                <div class="table-responsive-sm">
                                    <table class="table table-sm">
                                        ...
                                        <tbody id="tbody_gr_transportista"></tbody>
                                    </table>
                                </div>
                            </div>
                        ...
                        <div class="col-12">
                            <div class="col-12 text-end">
                                <a href="{{ route('admin.gr_transportista') }}" class="btn btn-secondary">Cancelar</a>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function load_cart() {
            $.ajax({
                url: "{{ route('admin.load_cart_gr_transportista') }}",
                method: 'POST',
                data: {
                    '_token': "{{ csrf_token() }}"
                },
                success: function(r) {
                    ...
                    $('#tbody_gr_transportista').html(r.html_cart);
                },
                dataType: 'json'
            });
        }
        ...
        $('body').on('click', '#form_save_to_product .btn-save', function() {
            ...
            $.ajax({
                url: "{{ route('admin.add_product_gr_transportista') }}",
                method: "POST",
                data: {
                    '_token': "{{ csrf_token() }}",
                    id: select_product,
                ...
                success: function(r) {
                    ...
                    $('#form_save_to_product .text-save').removeClass('d-none');
                    $('#form_save_to_product .text-saving').addClass('d-none');
                    $('#form_save_to_product input[name="cantidad"]').val('0');
        ...
        $('body').on('click', '.btn-delete-product', function() {
            ...
            let id = $(this).data('id');
            $.ajax({
                url: "{{ route('admin.delete_product_gr_transportista') }}",
                method: 'POST',
                data: {
                    '_token': "{{ csrf_token() }}",
                    id: id
        ...
        $('body').on('click', '#form_gr_transportista .btn-save', function() {
            event.preventDefault();
            let form = $('#form_gr_transportista').serialize();
            $.ajax({
                url: "{{ route('admin.save_gr_transportista') }}",
                method: "POST",
                data: form,
                beforeSend: function() {
                    $('#form_gr_transportista .btn-save').prop('disabled', true);
                    $('#form_gr_transportista .text-save').addClass('d-none');
                    $('#form_gr_transportista .text-saving').removeClass('d-none');
                },
                success: function(r) {
                    if (!r.status) {
                        $('#form_gr_transportista .btn-save').prop('disabled', false);
                        $('#form_gr_transportista .text-save').removeClass('d-none');
                        $('#form_gr_transportista .text-saving').addClass('d-none');
                    ...
                    $('#form_gr_transportista .text-save').removeClass('d-none');
                    $('#form_gr_transportista .text-saving').addClass('d-none');
                    $('#form_gr_transportista select[name="modo_pago"] option[value="1"]').prop('selected', true);
                    $('#form_gr_transportista')[0].reset();
