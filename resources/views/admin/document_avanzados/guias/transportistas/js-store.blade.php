<script>
    ...
    $('body').on('click', '.btn-confirm', function() {
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: "{{ route('admin.delete_transportistas') }}",
                    method: 'POST',
                    data: {
                        '_token': "{{ csrf_token() }}",
                        id: id
    ...
    $('body').on('click', '.btn-detail', function() {
        ...
        let id = $(this).data('id');
        $.ajax({
            url: "{{ route('admin.detail_transportistas') }}",
            method: 'POST',
            data: {
                '_token': "{{ csrf_token() }}",
                id: id
            ...
            success: function(r) {
                ...
                html_province = '';
                html_district = '';
                $(`#form_edit_client select[name="tipo_documento"] option[value="${r.transportistas.iddoc}"]`)
                    .prop('selected', true);
                switch (r.transportistas.iddoc) {
                    case 2:
                ...
                $(`#form_edit_client input[name="id"]`).val(r.transportistas.id);
                $(`#form_edit_client input[name="dni_ruc"]`).val(r.transportistas.dni_ruc);
                $(`#form_edit_client input[name="razon_social"]`).val(r.transportistas.nombres);
                $(`#form_edit_client input[name="direccion"]`).val(r.transportistas.direccion);
                $(`#form_edit_client input[name="mtc"]`).val(r.transportistas.mtc);
    ...
    $('body').on('click', '#form_edit_client .btn-store-client', function() {
        ...
        let form = $('#form_edit_client').serialize(),
            ...
            mtc = $('#form_edit_client input[name="mtc"]');
        ...
        if (mtc.val().trim() == '')
        mtc.addClass('is-invalid');
        else
        mtc.removeClass('is-invalid');

        if (tipo_documento.val() != '0' && dni_ruc.val().trim() != '' &&
        mtc.val().trim() != '' &&
            razon_social.val().trim() != '' && !(direccion.val().trim() == '' && !dni)) {
            $.ajax({
                url: "{{ route('admin.store_transportistas') }}",
                method: 'POST',
                data: form,
                beforeSend: function() {
